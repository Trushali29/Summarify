...
Jo, T. et al. [6] provided the idea of extracting keywords using backpropagation neural networks. It considers key features such as title, first and last sentence words, TF, IDF, and ITF that are provided to the neural network model. This concludes that the threshold of around 0.51 and 0.49 can approximately classify keywords (that provide relevant information) from non-keywords (that are not relevant enough). While Sinha A. et al. [5] have also used word embedding methods to convert the text into a numerical value using a fast-text technique that provides a vector representation of words generating a summary of limited length.
...
While graph-based methods such as the TextRank algorithm is an unsupervised graph-based text summarizing method that employs an extractive approach. Search engines like Google employ the PageRank algorithm to determine the ranking of online sites. The PageRank algorithm is the foundation of TextRank. The paper by Gunasundari S. et al. [14] discusses the TextRank approach with cosine similarity to avoid duplicate copies of sentences in the summary.
Other built-in modules, such as SPACY and NLP (natural language processing), are used to build extractive-based summary models, discussed in papers [15] and [16].
...
To achieve text summarization using GloVe a 3-Step approach is done.
	Load the pre-trained GloVe model. Each word in the embeddings is associated with a dense vector representation which is stored in a dictionary.
	For each sentence in the article a sentence embedding is created. To calculate sentence embeddings for each sentence, a sentence is split into words. It takes the vector representation of each words. If that word is not found a zero vector is used as a placeholder. The resulting sentence embedding consist the mean value of all the words as a dense vector representation.
	A cosine similarity matrix is created with take sentence embeddings as input. A row is represented as a sentences and column consists value of words. Each row value is summed up and a final score is generated for each row. All of the row values are sorted in the descending order and top N sentences is chosen for the summary. A sentence with a highest value is consider to be a part of the summary.
...
Steps to generate summaries using TextRank algorithm
	Creating a vocabulary which consist set of unique words from the article.
	Creating a co-occurrence matrix where each row represents a sentence and column represents a word in the vocabulary. The matrix is populated with word counts, meaning frequency of word occurring in each sentence.
	Cosine Similarity is used to calculate cosine similarity between sentences based on the co-occurrence matrix. 

cosine_similarity(A,B) = (A dot B) / (||A|| * ||B||)                  (4)

	TextRank similarity is applied on sentences to rank them based on their similarity. It runs for k iterations to update the sentence score.

Score(Sj) = (1-damping factor)+damping factor*∑(Si*Sj )                  (5)

	Finally, the scores generated from the algorithm sorted in descending order considering top N highest score sentences to generate summary.  
...
Domain	No. of Articles  and summaries
Business	510
Tech	401
Sport	511
Entertainment	386
...
Table 1: BBC news Dataset having number of articles in each domain
...
One more approach can be added is to only consider pronouns and nouns (actions words) increasing the performance of summary on different algorithms as shown in figure 3 below. Few algorithms performs better without POS tags too. In results section it provides those algorithms which works better with or without POS tags. 

4.3.3 Algorithms  

All three algorithms are extractive based approach. These takes input generated after pre-processing and feature extraction and compute probabilities of each term. These probabilities or score of each term tells whether or not a word can be a part of the summary. 

4.3.4 Sentence Scoring  

In this step, the top most N sentences are considered as a summary. To get important sentences probabilities or score of words from previous step is used. A sentence score is computed by aggregating all values of words. Finally, a top N sentence is chosen in descending order considering higher values. 
